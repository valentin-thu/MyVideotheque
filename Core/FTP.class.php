...
class Core_FTP {
	 ...
	 */
	/*
	 * public function nlist($directory = null){ $connexion = ftp_connect($this->_host, $this->_port); if(ftp_login($connexion, $this->_login, $this->_password)){ return ftp_nlist($connexion, $directory); } ftp_close($connexion); }
	 */
	...
	public function nlist($directory = null) {
		...
		foreach ( $array as $file ) {
			...
			$dir = explode ( '/', $file );
			if ($dir [count ( $dir ) - 1] == '.' || $dir [count ( $dir ) - 1] == '..')
				continue;
	...
	public function nlistFiles($directory = null, $start = false) {
		...
		if ($this->_exist !== false) {
			...
			foreach ( $this->_listeDossier as $dir => $value ) {
				...
				if ($nameDir [count ( $nameDir ) - 1] == '.' || $nameDir [count ( $nameDir ) - 1] == '..')
					continue;
